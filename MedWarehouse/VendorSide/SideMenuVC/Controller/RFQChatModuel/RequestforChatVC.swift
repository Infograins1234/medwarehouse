...
class RequestforChatVC: UIViewController, UITextViewDelegate  {
...
//MARK:-
extension RequestforChatVC:UIImagePickerControllerDelegate,UINavigationControllerDelegate{
...
//MARK:-
extension RequestforChatVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if Int(recieve?.sender_id ?? "0") == Int(ApplicationPreference.shared.getUserData().id ?? "0") {
            let cell = tableView.dequeueReusableCell(withIdentifier: "RequestforChatTableViewCell") as! RequestforChatTableViewCell
            cell.comment = recieve
            return cell
        }
        
        let cell = tableView.dequeueReusableCell(withIdentifier: "RequestforRecieveTableViewCell") as! RequestforRecieveTableViewCell
        cell.comment = recieve
        return cell
    }
...
extension RequestforChatVC{
    ...
    func getCommentList(){
        ...
        objWebServiceManager.requestPost(strURL: message_list, params: param, showIndicator: false, success: { (response) in
            //
            if let data = response as? [String : Any] {
